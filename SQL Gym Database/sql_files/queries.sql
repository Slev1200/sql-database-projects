...
SELECT day_of_week, ROUND(AVG(class_size), 1) AS avg_class_size FROM 
   (SELECT s.day_of_week, s.schedule_id, COUNT(e.enrollment_id) AS class_size FROM schedules s
    LEFT JOIN class_enrollments e ON s.schedule_id = e.schedule_id AND e.status IN ('enrolled', 'attended')
